"""
Symmetric to the REV gate: should TREND (MOM) buys also require being INSIDE the
iso-conviction curves (|slope|.R2 >= k*xabs)? REV improved by reserving fades for
names OUTSIDE the innermost curve. But MOM's base gate (ER>=0.40 & slope>0 & >50 &
>200) is already a strong trend filter that correlates with high slope.R2, so an
inside-curves floor may be redundant or even harmful (it removes moderate-
conviction names that ER already blessed, and thins deployment).

On top of the CURRENT live config (top-8 blend cut on MOM; REV = ER<=.35 & rsi2<15
& outside innermost curve), test adding a MOM floor at each of the 3 curves:
  base     MOM = ER>=.40 & slope>0 & >50 & >200                 (current live)
  mom_k1   base AND |slope|.R2 >= 0.18*xabs   (inside innermost)
  mom_k2   base AND |slope|.R2 >= 0.36*xabs   (inside middle)
  mom_k3   base AND |slope|.R2 >= 0.60*xabs   (inside outer, strictest)

A floor earns its keep only if it holds/improves Sharpe & CAGR without deepening
maxDD or starving deployment. Curve conviction uses the MAP 20-day slope (x2000)
& 20-day R2 - faithful to the live map.
"""
import sys
import time
...
def main():
    ...
    def route(c, j, mom_f):
        ...
        if e >= ER_HI and fin(m50[c][j]) and pv[c][j] > m50[c][j] and fin(SL63[c][j]) and SL63[c][j] > 0:
            if mom_f is None or conv(c, j) >= mom_f * XABS[j]:
                return "MOM"
            return None
        # REV = current live: ER<=.35 & rsi2<15 & outside innermost curve
    ...
    def sim(mom_f):
        ...
        for j in range(si, n - 1):
            if j in rd:
                mom = [c for c in present if route(c, j, mom_f) == "MOM" and fin(VOL[c][j]) and VOL[c][j] > 0]
                rev = [c for c in present if route(c, j, mom_f) == "REV" and fin(VOL[c][j]) and VOL[c][j] > 0]
                momn.append(len(mom))
                if len(mom) > TOPK:
        ...
        sc = slice(si + 1, n)
        return ret[sc], idx[sc].values, np.mean(momn), np.mean(dep)

    print(f"\nMOM inside-curves floor (on current live config). net@{COST:.0f}bps")
    print("            avgMOM  deploy | Sh 11-19/20-26  CAGR/maxDD   2Y")
    for lab, f in [("base", None), ("mom_k1 .18", 0.18), ("mom_k2 .36", 0.36), ("mom_k3 .60", 0.60)]:
        r, d, am, dp = sim(f)
        ...
        print(f"  {lab:11s}  {am:>4.1f}  {dp:>5.1%} | {s1:>4.2f}/{s2:>4.2f}  {cgf:>5.1%}/{ddf:>6.1%} {cg2:>5.1%}")
