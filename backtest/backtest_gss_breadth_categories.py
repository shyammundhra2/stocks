"""
Expand breadth within the book's OWN categories - subsectors, countries, bond
curve (only TLT today) - and test which actually raises returns. Subsectors +
countries add EQUITY DISPERSION (more distinct trends for the momentum sleeve),
unlike the prior bond-heavy add which just diluted. Isolate each category.

Configs (shipped adaptive config, 2007-26):
  current (36)
  +subsectors     equity dispersion within industries
  +countries      equity dispersion across geographies
  +bonds          curve + credit (gate-exempt, held through risk-off)
  +all
Report deployment, breadth, Sharpe/Sortino/CAGR/DD full + dev + crises.
All ETFs -> safe co-download. Full-history (pre-2008) names only.
"""
import sys
import time
...
SUBSECTORS = ["XBI", "XHB", "XRT", "KRE", "XOP", "OIH", "IYT", "GDX", "KIE", "FDN", "IHI", "SMH"]
COUNTRIES_NEW = ["FXI", "EWG", "EWU", "EWT", "EWW", "EWA", "EWC", "EWH", "EWS",
                 "EWP", "EWQ", "EWL", "EEM", "EFA", "EWM", "EWD"]
BONDS_NEW = ["IEF", "SHY", "IEI", "LQD", "HYG", "EMB", "TIP", "TLH", "MBB", "JNK"]
BOND_DEFENSIVE = set(BONDS_NEW)
...
def main():
    t0 = time.time()
    dl = sorted(set(BASE + SUBSECTORS + COUNTRIES_NEW + BONDS_NEW + ["BIL", "SPY"]))
    ...
    def full_hist(lst):
        out = []
        for c in lst:
            if c in close.columns and close[c].dropna().index.min() <= pd.Timestamp("2008-06-01"):
                out.append(c)
        return out
    sub = full_hist(SUBSECTORS); cty = full_hist(COUNTRIES_NEW); bnd = full_hist(BONDS_NEW)
    print(f"full-history subsectors: {sub}")
    print(f"full-history countries:  {cty}")
    print(f"full-history bonds:      {bnd}\n")
    ...
    configs = {
        ...
        "+subsectors": (BASE + sub, DEFENSIVE_ASSETS),
        "+countries": (BASE + cty, DEFENSIVE_ASSETS),
        "+bonds": (BASE + bnd, DEFENSIVE_ASSETS | (BOND_DEFENSIVE & set(bnd))),
        "+all": (BASE + sub + cty + bnd, DEFENSIVE_ASSETS | (BOND_DEFENSIVE & set(bnd))),
    }
    results = {}
    ...
    for name in configs:
        ...
        print(f"{name:>14s}: universe {len(configs[name][0]):>3d}  deployment {dep:>3.0%}  avg {nm:.1f} names")
    print()
