"""
Should there be a max-deployed-capital cap? Expanding to 52 names raised
deployment ~40%->68%; in a broad rally it could spike higher (many correlated
equity names all trending) - right before the drawdown. The vol cap should
contain this but we found it barely binds. Test:
  1. Deployment DISTRIBUTION over time (avg / 95th / max) - does it spike?
  2. Explicit max-deploy cap {none, 85%, 75%, 65%}: scale the whole book down
     if sum(weights) exceeds the cap (excess -> cash). Helps risk-adjusted
     return, or just cash drag?
52-name universe, shipped config, 2011-26 + crises.
"""
import sys
import time
...
DEPLOY_CAPS = [None, 0.85, 0.75, 0.65]
...
def main():
    ...
    def sim(max_deploy):
        ...
        sc = slice(si + 1, n)
        return ret[sc] - turn[sc] * (COST / 1e4), idx[sc].values, np.array(deps)

    results = {c: sim(c) for c in DEPLOY_CAPS}
    _, _, deps0 = results[None]
    print(f"\nDeployment distribution (no cap, 52 names): avg {deps0.mean():.0%}, "
          f"median {np.median(deps0):.0%}, 95th {np.percentile(deps0, 95):.0%}, max {deps0.max():.0%}")
    print(f"  fraction of days >75% deployed: {(deps0 > 0.75).mean():.0%}   >85%: {(deps0 > 0.85).mean():.0%}\n")
    for lab, lo, hi in [("2020 COVID", "2020-02-01", "2020-04-30"),
        ...
        for c in DEPLOY_CAPS:
            r, dts, _ = results[c]; sh, so, cg, dd = perf(r, dts, lo, hi)
            lbl = "no cap" if c is None else f"cap {int(c*100)}%"
            print(f"  {lbl:12s} {sh:>7.2f} {so:>8.2f} {cg:>7.1%} {dd:>7.1%}")
        print()
