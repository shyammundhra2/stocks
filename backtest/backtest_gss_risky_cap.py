"""
Should very-risky (high-vol) positions be capped tighter - to $20k (4%)?
Inverse-vol sizing already downweights high-vol names (weight ~ 1/vol), so
test whether an explicit vol-conditional cap adds anything. A name whose 63d
annualized vol exceeds a threshold is capped at 4% ($20k on 500k) instead of
the usual MOM 7.5% / REV 5%.

Configs (52-name book, shipped config, 2011-26):
  current        - MOM 7.5% / REV 5%, no vol-conditional cap
  risky>35%->4%  - names with ann vol >35% capped at 4%
  risky>45%->4%  - threshold 45%
  risky>60%->4%  - threshold 60% (only the wildest: crypto etc.)
Report Sharpe/Sortino/CAGR/DD + how often the risky cap actually binds.
"""
import sys
import time
...
MOM_CAP, REV_CAP, RISKY_CAP, COST, BUF = 0.075, 0.05, 0.04, 5.0, 0.03
BASE = list(TREND_ASSETS.keys())
# (threshold on ANNUALIZED 63d vol, label)
CONFIGS = [(None, "current"), (0.35, "risky>35%->4%"), (0.45, "risky>45%->4%"), (0.60, "risky>60%->4%")]
...
def main():
    ...
    for c in present:
        ...
        VOL[c] = close[c].pct_change().rolling(63).std().reindex(idx).values   # daily vol
    ...
    def cap_for(c, j, kind, thr):
        base = REV_CAP if kind == "REV" else MOM_CAP
        if thr is not None:
            annv = VOL[c][j] * np.sqrt(252)
            if fin(annv) and annv > thr:
                return min(base, RISKY_CAP)
        return base
    ...
    def sim(thr):
        ...
        for j in range(si, n - 1):
            if j in rd:
                ...
                if tot > 0:
                    for c in sel:
                        cp = cap_for(c, j, kind[c], thr)
                        if thr is not None and sel[c] / tot > cp and VOL[c][j] * np.sqrt(252) > (thr or 9):
                            binds += 1
                        tg[c] = min(sel[c] / tot, cp)
                    s2 = sum(tg.values())
                    if s2 > 0:
                        tg = {c: v / s2 for c, v in tg.items()}
                        tg = {c: min(v, cap_for(c, j, kind[c], thr)) for c, v in tg.items()}
        ...
        sc = slice(si + 1, n)
        return ret[sc] - turn[sc] * (COST / 1e4), idx[sc].values, binds / max(reb, 1)

    results = {lbl: sim(thr) for thr, lbl in CONFIGS}
    for thr, lbl in CONFIGS:
        print(f"{lbl:16s} risky-cap binds ~{results[lbl][2]:.1f} names/rebalance")
    print()
    for lab, lo, hi in [("2020 COVID", "2020-02-01", "2020-04-30"),
        ...
        for thr, lbl in CONFIGS:
            r, dts, _ = results[lbl]; sh, so, cg, dd = perf(r, dts, lo, hi)
            print(f"  {lbl:16s} {sh:>7.2f} {so:>8.2f} {cg:>7.1%} {dd:>7.1%}")
        print()
