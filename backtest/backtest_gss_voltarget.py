"""
Edge hunt on the shipped config. Two candidates with real priors:

  volT  - VOL-TARGET the book: the shipped config runs chronically cold
          (~12% vol vs the 15% cap, ~56% deployed). At each rebalance scale
          all weights by target_vol / realized book vol (63d), capped at
          sum(w)<=1 (no leverage) and per-name caps re-applied. This is
          Moreira-Muir vol management: use the unused budget in calm tape,
          auto-shrink in turbulence.
  def+  - CRISIS REALLOCATION: during risk-off the freed equity capital sits
          in BIL; instead raise the defensive (GLD/SLV/DBC/TLT) per-name cap
          15% -> 25% while risk-off, letting the CTA-style crisis trades
          (2008 TLT/gold) use the budget.
  both  - volT + def+

Base = shipped: router, ToM, eq-only buffered gate, inverse-vol, 15%/5% caps,
BIL on cash, net@5bps. Full 2007-26 + windows, with Sortino.
"""
import sys
import time
...
CAP, REV_CAP, DEF_CAP_RISKOFF, COST, BUF, TARGET = 0.15, 0.05, 0.25, 5.0, 0.03, 0.15
...
def main():
    ...
    rets_df = close[present].pct_change()
        ...
        for j in range(si, n - 1):
            if j in rd:
                ...
                def name_cap(c):
                    if kind[c] == "REV":
                        return REV_CAP
                    if def_boost and riskoff and c in DEFENSIVE_ASSETS:
                        return DEF_CAP_RISKOFF
                    return CAP

                tot = sum(sel.values()); tg = {}
                if tot > 0:
                    tg = {c: min(sel[c] / tot, name_cap(c)) for c in sel}
                    s2 = sum(tg.values())
                    if s2 > 0:
                        tg = {c: v / s2 for c, v in tg.items()}
                        tg = {c: min(v, name_cap(c)) for c, v in tg.items()}
                if vol_target and tg:
                    bv = book_vol(tg, j)
                    if bv > 0:
                        s_ = min(TARGET / bv, 1.0 / max(sum(tg.values()), 1e-9))
                        tg = {c: v * s_ for c, v in tg.items()}
                        tg = {c: min(v, name_cap(c)) for c, v in tg.items()}   # re-clip
    ...
    configs = [("base (shipped)", (False, False)), ("volT", (True, False)),
               ("def+", (False, True)), ("volT+def+", (True, True))]
    ...
    for lab, lo, hi in WINDOWS:
        ...
        for nm, _ in configs:
            r, dts = results[nm]; sh, so, cg, dd = stats(r, dts, lo, hi)
            print(f"  {nm:15s} {sh:>7.2f} {so:>8.2f} {cg:>7.1%} {dd:>7.1%}")
        sh, so, cg, dd = stats(spyr, results["base (shipped)"][1], lo, hi)
        print(f"  {'SPY':15s} {sh:>7.2f} {so:>8.2f} {cg:>7.1%} {dd:>7.1%}\n")
