"""
Per-name cap sweep on the shipped config: should the MOM cap be 15% ($75k on
$500k) or lower? The cap is a concentration limit, not a per-name Kelly (no
per-asset edge to estimate). Question: does forcing more diversification -
lower cap -> more names, smaller each -> improve risk-adjusted return, or does
the breadth-limited universe just push the freed capital to cash?

Sweep MOM cap in {15,12.5,10,7.5,5}% (REV stays 5%). Shipped config: adaptive
router, ToM rebalance, equity-only buffered gate, inverse-vol sizing, vol-
target scalar, BIL on cash, net@5bps. Full 2007-26 + crises, Sharpe/Sortino/
maxDD + avg deployment + $ per name.
"""
import sys
import time
...
WIN, ER_HI, ER_LO, RSI_BUY, REV_CAP, COST, BUF, PV = 20, 0.40, 0.35, 15.0, 0.05, 5.0, 0.03, 500_000
MOM_CAPS = [0.15, 0.125, 0.10, 0.075, 0.05]
...
def main():
    ...
    rv21 = pd.Series(close["SPY"].pct_change().rolling(21).std().values * np.sqrt(252))
    rv_med = rv21.rolling(252, min_periods=60).median().values; rv21 = rv21.values
    ...
    def sim(mom_cap):
        ...
        for j in range(si, n - 1):
            ...
            dep.append(sum(w.values())); nnames.append(sum(1 for v in w.values() if v > 0.001))
            s = 0.
            for c in present:
        ...
        sc = slice(si + 1, n)
        return ret[sc] - turn[sc] * (COST / 1e4), idx[sc].values, np.mean(dep), np.mean(nnames)

    results = {c: sim(c) for c in MOM_CAPS}
    print(f"{'MOM cap':>8s} {'$cap':>7s} {'deploy':>7s} {'#names':>7s}")
    for c in MOM_CAPS:
        _, _, dep, nm = results[c]
        print(f"{int(c*100):>6d}% {'$'+format(int(c*PV/1000),'d')+'k':>7s} {dep:>6.0%} {nm:>7.1f}")
    print()
    for lab, lo, hi in WINDOWS:
        print(f"== {lab} ==   {'Sharpe':>7s} {'Sortino':>8s} {'CAGR':>7s} {'maxDD':>7s}")
        for c in MOM_CAPS:
            r, dts, _, _ = results[c]; sh, so, cg, dd = stats(r, dts, lo, hi)
            print(f"  MOM cap {int(c*100):>2d}%      {sh:>7.2f} {so:>8.2f} {cg:>7.1%} {dd:>7.1%}")
        print()
